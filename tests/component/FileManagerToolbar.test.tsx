...
describe('FileManagerToolbar', () => {
  it('shows only the active container label when a stale path is outside its root', () => {
    const view = render(<I18nProvider><FileManagerToolbar rootDir="c-new" rootLabel="/home/workspace/c-new" currentPath="c-old" {...actions} /></I18nProvider>);
    const toolbar = within(view.container);
    expect(toolbar.getByRole('button', { name: '/home/workspace/c-new' })).toBeInTheDocument();
    expect(toolbar.queryByText('c-old')).not.toBeInTheDocument();
  });

  it('renders only root-relative child segments', () => {
    const view = render(<I18nProvider><FileManagerToolbar rootDir="c-demo" rootLabel="/home/workspace/c-demo" currentPath="c-demo/src/components" {...actions} /></I18nProvider>);
    const toolbar = within(view.container);
    expect(toolbar.getByRole('button', { name: '/home/workspace/c-demo' })).toBeInTheDocument();
