...
describe('ServicesPanel', () => {
  it('opens an in-page preview and copies ports without exposing external preview links', async () => {
    ...
    const { container } = render(<I18nProvider><ServicesPanel ports={[{ port: 5173, url: 'https://5173.example.webcontainer-api.io' }]} processes={[]} containerName="demo" onPreview={onPreview} onKillProcess={vi.fn()} /></I18nProvider>);
  ...
  it('sanitizes internal paths in process commands', () => {
    render(<I18nProvider><ServicesPanel ports={[]} containerName="demo" processes={[{ id: 'proc-1', sessionId: 's-1', runId: 'r-1', containerId: 'c-1', command: 'node /home/sunam/.sunam/workspaces/c-1/server.js', isRunning: true, output: '', cursor: 0 }]} onPreview={vi.fn()} onKillProcess={vi.fn()} /></I18nProvider>);
  ...
  it('shows clipboard failures instead of using a legacy copy fallback', async () => {
    ...
    render(<I18nProvider><ServicesPanel ports={[{ port: 3000, url: 'https://3000.example.test' }]} processes={[]} containerName="demo" onPreview={vi.fn()} onKillProcess={vi.fn()} /></I18nProvider>);
