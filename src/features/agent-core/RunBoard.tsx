    ...
    <div className="task-list-content" aria-hidden={!isExpanded} inert={!isExpanded}>
      <div className="task-list-scroll">
        ...
        {children.length > 0 && <div className="task-list-subagents">{children.map((child) => {
          ...
          return <details key={child.id} className="task-list-subagent" onToggle={(event) => {
            if (!event.currentTarget.open || loadedChildRuns.current.has(child.id) || loadingChildRuns.current.has(child.id)) return;
          ...
          }}><summary><Bot size={14} /><strong>{child.agentRole}</strong><span>{child.phase}</span><small>{child.delegatedTaskId}</small></summary><div><p>{child.finalSummary || child.summary || child.task.objective}</p>{child.task.evidence.length > 0 && <p>{child.task.evidence.slice(-3).join(' · ')}</p>}<small>{child.modelTurns}/{child.budget.maxModelTurns} turns · {child.toolCalls}/{child.budget.maxToolCalls} tools · {childTools.length} records</small>{transcript.length > 0 && <div className="task-list-subagent-transcript"><strong>{t('agent.transcript')}</strong>{transcript.map((message, index) => <div key={`${message.role}:${message.tool_call_id ?? index}`}><span>{message.name || message.role}</span><pre>{message.content.slice(0, 600)}</pre></div>)}</div>}</div></details>;
        })}</div>}
