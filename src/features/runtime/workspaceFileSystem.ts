...
export class WorkspaceFileSystem {
  ...
  async materialize(containerId: string, path: string, contents: Uint8Array): Promise<WorkspaceChangeSummary> {
    const target = resolveContainerPath(containerId, path);
    let beforeBytes = 0;
    let kind: 'created' | 'updated' = 'created';
    try {
      beforeBytes = (await this.webcontainer.fs.readFile(target)).byteLength;
      kind = 'updated';
    } catch (error) {
      if (!isNotFoundError(error)) throw error;
    }
    const parent = target.slice(0, target.lastIndexOf('/')) || getContainerRoot(containerId);
    await this.webcontainer.fs.mkdir(parent, { recursive: true });
    await this.webcontainer.fs.writeFile(target, contents);
    return { path, kind, beforeBytes, afterBytes: contents.byteLength };
  }
}
