...
export function TerminalTabs({ activeTab, onTabChange, layoutState, onLayoutChange }: TerminalTabsProps) {
  const { t } = useI18n();
  return (
    <div className="dual-terminal-tabs">
...
export function CollapsedTerminalNav({ activeTab, onTabChange, onExpand }: { activeTab: TerminalTab; onTabChange: (tab: TerminalTab) => void; onExpand: () => void }) {
  const { t } = useI18n();
  return <div className="desktop-only-btn collapsed-terminal-nav">{tabDefinitions.map(([tab, Icon, label]) => <button key={tab} className={`right-sidebar-btn ${activeTab === tab ? 'active' : ''}`} onClick={() => { onTabChange(tab); onExpand(); }} title={t(label)}><Icon size={20} /></button>)}</div>;
}
