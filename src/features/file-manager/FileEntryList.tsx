...
import { Check, FileText, Folder, FolderOpen, FolderUp, MoreVertical, X } from 'lucide-react';
...
interface FileEntryListProps {
  ...
  onTouchStart: (event: TouchEvent, entry: FileEntry) => void;
...
export function FileEntryList(props: FileEntryListProps) {
  ...
  return <div className="fm-list" ref={props.listRef} onClick={props.onClearSelection}>
    ...
    {props.entries.map((entry, index) => <div key={entry.name} data-fm-drop-target={entry.isDirectory ? entry.name : undefined} className={`fm-item ${props.dragOverFolder === entry.name ? 'drag-over' : ''} ${props.selectedItem === entry.name ? 'selected' : ''}`} style={{ '--fm-item-index': Math.min(index, 8) } as CSSProperties} onClick={(event) => props.onItemClick(event, entry)} onDoubleClick={(event) => props.onItemDoubleClick(event, entry)} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); props.onOpenContextMenu(entry, event.clientX, event.clientY); }} onTouchStart={(event) => props.onTouchStart(event, entry)} onTouchEnd={props.onTouchEnd} onTouchMove={props.onTouchMove} onTouchCancel={props.onTouchCancel} draggable={!props.renamingEntry} onDragStart={(event) => props.onDragStart(event, entry)} {...(entry.isDirectory ? { onDragOver: (event: DragEvent) => props.onFolderDragOver(event, entry.name), onDragLeave: props.onFolderDragLeave, onDrop: (event: DragEvent) => props.onFolderDrop(event, entry.name) } : {})}>
      ...
      <button className="fm-toolbar-btn fm-item-menu" aria-label={`${t('files.actions')}: ${entry.name}`} title={t('files.actions')} onClick={(event) => { event.stopPropagation(); const rect = event.currentTarget.getBoundingClientRect(); props.onOpenContextMenu(entry, rect.right, rect.bottom); }}><MoreVertical size={16} /></button>
    </div>)}
  </div>;
}
