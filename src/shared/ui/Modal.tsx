...
export function Modal({ title, onDismiss, labelledBy = 'sunam-modal-title', children }: ModalProps) {
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onDismiss?.(); }}>
    <section role="dialog" aria-modal="true" aria-labelledby={labelledBy} className="settings-modal-content motion-pop-in">
