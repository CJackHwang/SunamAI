 ...
 * R5：内置皮套的系统提示词全文。
 *
 * 只被懒加载路径消费（prompt.ts 随 ConfiguredPage、皮套编辑面板随 SettingsPage），
 * 不 import 进初始 bundle——旧实现把这些长文案放在 prompt.ts（ConfiguredPage 懒加载
 * chunk）里，拆分后保持同样的懒加载语义。
 */

export const BUILTIN_PERSONA_PROMPTS: Record<string, string> = {
