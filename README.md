...
- 支持文件上传、下载、编辑、移动与预览。
- v2 会话、容器、Agent Run/checkpoint、终端输出和文件系统快照在本浏览器的版本化本地数据库中恢复；数据管理 API 仅保留为后续能力接口，当前不提供设置页入口。
- 内置中文、英文与日文界面，支持安装为 PWA。
- 使用响应式界面，适配桌面端开发场景。
...
Sunam 是纯前端应用。当前配置（包括 API 服务地址、模型和 API Key）存储在当前浏览器的本地存储中，v2 工作区数据位于同一浏览器的 IndexedDB，并由浏览器直接请求你指定的模型服务。

- 不要在公共设备或多人共用的浏览器配置个人 API Key。
- 本地导出的 v2 备份可能含源代码、终端输出和工作区文件；导出、传输或分享前请自行检查其中的敏感信息。
...
- [Agent Core v2 设计与后续路线](docs/agent-v2-design.md)
